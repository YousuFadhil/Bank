...
<head>
	...
	<title>Add New Client code</title>
</head>
...
<body>
	...
	if (count($errors) == 0) {
		...
		$q = "insert into tblclient values('$name','$address','$phone','$account','$loans','$id','$sector','$email')";
		mysqli_query($c, $q) or die("Error in $q" . mysqli_error($c));
		// Display
		echo "<h3>New client details are successfully added to the database</h3>";
		...
		echo "<br><br>";
		// Link
		echo "<a href='clerk.php'>Go back to Clerk Page</a>";
